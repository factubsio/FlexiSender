...
export function sendManual(): void {
  ...
  if (!cmd) return;
  state.cmdHistory.unshift(cmd); state.histIdx = -1; inp.value = '';
  import('./connection').then(c => c.sendCmd(cmd));
}

export function handleConInput(e: KeyboardEvent): void {
  const inp = document.getElementById('conInput') as HTMLInputElement;
  if (e.key === 'Enter') { sendManual(); return; }
  if (e.key === 'ArrowUp') { state.histIdx = Math.min(state.histIdx + 1, state.cmdHistory.length - 1); inp.value = state.cmdHistory[state.histIdx] || ''; }
  if (e.key === 'ArrowDown') { state.histIdx = Math.max(state.histIdx - 1, -1); inp.value = state.histIdx >= 0 ? state.cmdHistory[state.histIdx] : ''; }
}
